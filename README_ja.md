...
| **Hermes** | プラグイン | `bash adapters/hermes.sh` / `curl | bash`（[セットアップ](#hermes-セットアップ)） |
...
[Hermes Agent](https://github.com/NousResearch/hermes-agent) 用のネイティブ Python プラグイン。Nous Research 製 CLI ファーストの自律型 AI エージェントです。シェルラッパーやファイルシステムウォッチャーは不要 — プラグインは Hermes のライフサイクルフックシステムに直接登録されます。

**セットアップ：**

1. peon-ping ランタイムを先にインストール（`curl -fsSL https://raw.githubusercontent.com/PeonPing/peon-ping/main/install.sh | bash`）

2. Hermes アダプターインストーラーを実行：
...
3. Hermes を再起動。プラグインは起動時に自動検出されます。

**イベントマッピング：**

| Hermes フック | CESP カテゴリ | トリガー |
|---|---|---|
| `on_session_start` | `session.start` | 新セッション開始 |
| `pre_tool_call` (clarify) | `input.required` | Agent がユーザー入力を待機中 |
| `post_tool_call` (成功) | `task.complete` | ツール呼び出し完了 |
| `post_tool_call` (エラー) | `task.error` | ツール呼び出し失敗 |
| `on_session_end` | `session.end` | セッション終了 |

**設定**（すべて環境変数経由）：

| 変数 | デフォルト | 説明 |
|---|---|---|
| `CESP_PACKS_DIR` | `~/.claude/hooks/peon-ping/packs` | サウンドパックディレクトリ |
| `CESP_ACTIVE_PACK` | (直近にインストールされたもの) | パック名で明示指定 |
| `CESP_VOLUME` | `0.5` | マスター音量 (0.0–1.0) |
| `CESP_DEBOUNCE` | `0.5` | カテゴリごとの最小サウンド間隔秒数 |
| `CESP_PLATFORMS` | `cli` | カンマ区切り：`cli`、`gateway`、`cron` |
