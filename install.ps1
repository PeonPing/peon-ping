    ...
    switch -Regex ($Command) {
        ...
        "^--status$" {
            try {
                ...
                $statusState = if ($cfg.enabled) { "ENABLED" } else { "PAUSED" }
                $packName = if ($cfg.default_pack) { $cfg.default_pack } elseif ($cfg.active_pack) { $cfg.active_pack } else { "peon" }
                Write-Host "peon-ping: $statusState | pack: $packName | volume: $($cfg.volume)" -ForegroundColor Cyan
                # Show path_rules info
                $rules = @()
                if ($cfg.path_rules) { $rules = @($cfg.path_rules) }
                if ($rules.Count -gt 0) {
                    # Check if any rule matches current directory
                    $activeRule = $null
                    foreach ($r in $rules) {
...
# --- Pick a sound ---
# Override hierarchy: session_override > path_rules > pack_rotation > default_pack
...
if ($rotationMode -eq "agentskill" -or $rotationMode -eq "session_override") {
    ...
    if ($sessionPacks.ContainsKey($sessionId) -and $sessionPacks[$sessionId]) {
        ...
        if ($candidate -and (Test-Path $candidateDir -PathType Container)) {
            ...
            $activePack = if ($pathRulePack) { $pathRulePack } else { $activePack }
