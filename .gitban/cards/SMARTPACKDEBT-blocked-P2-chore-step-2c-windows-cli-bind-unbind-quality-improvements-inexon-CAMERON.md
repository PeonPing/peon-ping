...
| **3. Make Changes** | Added `Update-PeonConfig` helper, `bind`/`unbind`/`bindings` subcommands under `--packs`, `ExtraArgs` param, `--install` with progress feedback, updated `--help` text. | - [x] Changes are implemented. |
| **4. Test/Verify** | 222 Pester tests pass (18 new). E2E tests cover bind config mutation and unbind removal. | - [x] Changes are tested/verified. |
...
| Task | Detail/Link |
| :--- | :--- |
| **Changes Made** | Added `Update-PeonConfig` shared helper, `bind`/`unbind`/`bindings` subcommands with `--install`/`--pattern` flags, `ExtraArgs` param, progress feedback, 18 Pester tests |
| **Files Modified** | `install.ps1`, `tests/adapters-windows.Tests.ps1` |
| **Pull Request** | |
| **Testing Performed** | `Invoke-Pester` — 222 passed, 0 failed (18 new tests for helper, bind/unbind/bindings CLI, and E2E config mutation) |
