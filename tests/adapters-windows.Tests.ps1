...
Describe "Embedded peon.ps1 Hook Script" {
    ...
    It "defaults to active_pack when no session assignment" {
        $script:peonHookContent | Should -Match '\$config\.active_pack'
    }
...
Describe "path_rules: Runtime Matching Engine" {
    ...
    It "reads default_pack with active_pack fallback" {
        $script:peonHookContent | Should -Match 'if \(\$config\.default_pack\).*elseif \(\$config\.active_pack\)'
    }
}
