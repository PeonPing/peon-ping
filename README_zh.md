...
| **Hermes** | 插件 | `bash adapters/hermes.sh` / `curl | bash`（[设置](#hermes-设置)） |
...
[Hermes Agent](https://github.com/NousResearch/hermes-agent) 的官方 Python 插件，由 Nous Research 开发的 CLI 优先自治 AI Agent。无需 shell 包装器或文件系统监视 —— 插件直接注册到 Hermes 的生命周期钩子系统。

**设置步骤：**

1. 先安装 peon-ping 运行时（`curl -fsSL https://raw.githubusercontent.com/PeonPing/peon-ping/main/install.sh | bash`）
...
3. 重启 Hermes，插件会在启动时自动发现。

**事件映射：**

| Hermes 钩子 | CESP 分类 | 触发条件 |
|---|---|---|
| `on_session_start` | `session.start` | 新会话开始 |
| `pre_tool_call` (clarify) | `input.required` | Agent 等待用户输入 |
| `post_tool_call` (成功) | `task.complete` | 工具调用完成 |
| `post_tool_call` (错误) | `task.error` | 工具调用失败 |
| `on_session_end` | `session.end` | 会话关闭 |

**配置**（通过环境变量）：

| 变量 | 默认值 | 说明 |
|---|---|---|
| `CESP_PACKS_DIR` | `~/.claude/hooks/peon-ping/packs` | 音效包目录 |
| `CESP_ACTIVE_PACK` | (最近安装的) | 按名称指定使用的包 |
| `CESP_VOLUME` | `0.5` | 主音量 (0.0–1.0) |
| `CESP_DEBOUNCE` | `0.5` | 分类间音效最小间隔秒数 |
| `CESP_PLATFORMS` | `cli` | 逗号分隔：`cli`、`gateway`、`cron` |
